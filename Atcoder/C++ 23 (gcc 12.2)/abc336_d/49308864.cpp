...
void solve()
{
    ...
    loop(i,n)
    {
        while(pqu.size() && pqu.top().first == -i)
        {
            while( (ll)(i - qu.size()) < (ll)(pqu.top().second) )
            {
                //cout<<(i - qu.size())<<endl;
                //cout<<(pqu.top().second)<<endl;
                //cout<<"bool"<<((i - qu.size()) < (pqu.top().second))<<endl;
                qu.pop();
            }
            pqu.pop();
        }
        qu.push(arr[i]);
        pqu.push({-i-arr[i],i-arr[i]+1});
        ans = max(ans, qu.size()/2 + (qu.size()%2));
    }
    cout<<ans<<"\n";
}
int main()
